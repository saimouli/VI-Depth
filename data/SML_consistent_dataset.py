...
class SML_consistent_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.add_noise:
            # Add noise to sparse points
            fx, fy, cx, cy = intrinsics[0, 0], intrinsics[1, 1], intrinsics[0, 2], intrinsics[1, 2]
            validity_map_bool = tgt_sparse_depth > 0
            input_sparse_depth_valid = (tgt_sparse_depth < 5.0) * (tgt_sparse_depth > 0.2)
            input_sparse_depth_valid = input_sparse_depth_valid.astype(bool)
        
            # Backproject sparse points
