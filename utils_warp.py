...
def cam2pixel2(cam_coords, proj_c2p_rot, proj_c2p_tr, padding_mode):
    ...
    if padding_mode == "zeros":
        ...
        X_norm[X_mask] = (
            2  # make sure that no point in warped image is a combination of im and gray
        )
    ...
    return (
        pixel_coords.reshape(b, h, w, 2),
        Z.reshape(b, 1, h, w),
        pixel_coords2.reshape(b, h, w, 2),
    )
...
def pose_mof2mat_v1(mof, rotation_mode="euler"):
    ...
    transform_mat = transform_mat.reshape(bs, hh, ww, 3, 4).permute(
        0, 3, 4, 1, 2
    )  # [B, 3, 4, H, W]
    # pdb.set_trace()
    return transform_mat
...
def inverse_warp2(
    ...
    cam_coords = pixel2cam(
        depth.squeeze(1), intrinsics.inverse()
    )  # D * K_inv * X, [B,3,H,W]
    ...
    rot, tr = proj_cam_to_src_pixel[:, :, :3], proj_cam_to_src_pixel[:, :, -1:]
    src_pixel_coords, computed_depth, _ = cam2pixel2(
    ...
    if np.array(torch.__version__[:3]).astype(float) >= 1.3:
        projected_depth = F.grid_sample(
            ref_depth, src_pixel_coords, padding_mode=padding_mode, align_corners=True
        ).clamp(min=1e-3)
    else:
        projected_depth = F.grid_sample(
            ref_depth, src_pixel_coords, padding_mode=padding_mode
        ).clamp(min=1e-3)

    return projected_img, valid_mask, projected_depth, computed_depth


def transform_scale_consistent_depth(
    depth, pose, intrinsics, rotation_mode="euler", padding_mode="zeros"
):
    """
...
def depth2flow(
    ...
    proj_cam_to_src_pixel = intrinsics @ pose_mat  # [B,3,4]
    rot, tr = proj_cam_to_src_pixel[:, :, :3], proj_cam_to_src_pixel[:, :, -1:]
    flow_grid, valid_mask = cam2homo(
        cam_coords, rot, tr, padding_mode
    )  # [B,H,W,2], [B,1,H,W]
    mgrid_np = np.expand_dims(
        np.mgrid[0:ww, 0:hh].transpose(2, 1, 0).astype(np.float32), 0
...
def forward_warp(
    ...
    pcoords, Z = cam2pix_trans(
        cam_coords, pose_mat, intrinsics
    )  # [B,uH,uW,2], [B,1,uH,uW]
    ...
    for coo, z in zip(pcoords, Z):
        ...
        idx[1][idx[1] > ww - 1] = ww
        _idx, _val = coalesce(
            idx, 1 / val, m=hh + 1, n=ww + 1, op="max"
        )  # Cast an index with maximum-inverse-depth: we do NOT interpolate points! >> errors near boundary
        depth_w.append(
            1
            / torch.sparse.FloatTensor(
                _idx, _val, torch.Size([hh + 1, ww + 1])
            ).to_dense()[:-1, :-1]
        )  # normal depth
        fw_val.append(
            1
...
def cam2pixel_mof(cam_coords, proj_c2p_rot, proj_c2p_tr, padding_mode):
    ...
    if proj_c2p_rot is not None:
        pcoords = (
            c2p_rot_flat.permute(0, 2, 1, 3) * cam_coords_flat
        )  # [B, 3, 3, H*W] // (K * P) * (D_tgt * K_inv)
    ...
    return (
        pixel_coords.reshape(bs, hh, ww, 2),
        Z.reshape(bs, 1, hh, ww),
        pixel_coords2.reshape(bs, hh, ww, 2),
    )


def inverse_warp_mof(
    img, depth, ref_depth, motion_field, intrinsics, padding_mode="zeros"
):
    """
    Inverse warp a source image to the target image plane.
    Args:
    ...
    transform_field = transform_field.permute(0, 3, 4, 1, 2).reshape(
        bs, -1, 3, 4
    )  # [B, N, 3, 4]

    # Get projection matrix for tgt camera frame to source pixel frame
    proj_cam_to_src_pixel = (
        intrinsics.reshape(bs, 1, 3, 3) @ transform_field
    )  # [B, N, 3, 4]
    ...
    """
    src_pixel_coords, computed_depth, flow_grid = cam2pixel_mof(
        cam_coords, rot, tr, padding_mode
    )  # [B,H,W,2]
    if np.array(torch.__version__[:3]).astype(float) >= 1.3:
        projected_img = F.grid_sample(
...
def flow_warp(img, flo):
    ...
    mgrid_np = np.expand_dims(
        np.mgrid[0:gw, 0:gh].transpose(0, 2, 1).astype(np.float32), 0
    ).repeat(bs, axis=0)
...
def forward_warp_test(
    ...
    pose_mat = pose  # pose_vec2mat(pose, rotation_mode)  # [B, 3, 4]
    pcoords, Z = cam2pixel(
        cam_coords, pose_mat[:, :3, :3], pose_mat[:, :, 3:], padding_mode
    )  # [B, H, W, 2], [B, 1, H, W]

    if torch.__version__[:3] >= "1.3":
        projected_img = F.grid_sample(
            img, pcoords, padding_mode=padding_mode, align_corners=True
        )
    else:
